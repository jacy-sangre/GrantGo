  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      const [profileRes, scholarshipsRes, bookmarkRes] = await Promise.all([
        supabase.from("profiles").select("full_name, role, region_id").eq("id", userId).single(),
    ...
    <main className="space-y-6">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold text-slate-900 sm:text-4xl">
              {profile?.full_name ? `Hi, ${profile.full_name}` : "Your student dashboard"}
            </h1>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <a href="/" className="inline-flex items-center gap-2 rounded-full bg-brand px-4 py-2 text-sm font-medium text-white transition hover:bg-blue-600">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent className="text-slate-900">{profile?.role || "student"}</CardContent>
        </Card>
      </section>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <a href="/" className="text-sm font-medium text-brand hover:text-blue-600">
